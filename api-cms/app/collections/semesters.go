...
func (u *Semester) CollectionName() string {
	return "semester"
}

func (u *Semester) Find(filter interface{}, opts ...*options.FindOptions) (Semesters, error) {
	ctx, cancel := context.WithTimeout(context.Background(), QueryTimeOut)
	defer cancel()
	data := make(Semesters, 0)
...
func (u *Semester) First(filter interface{}) error {
	ctx, cancel := context.WithTimeout(context.Background(), QueryTimeOut)
	defer cancel()
