...
type Intern struct {
	...
	Student primitive.ObjectID `bson:"studentid" json:"studentid"`
...
func (u *Intern) CollectionName() string {
	return "interns"
}

func (u *Intern) Find(filter interface{}, opts ...*options.FindOptions) (Interns, error) {
...
func (u *Intern) First(filter interface{}) error {
	ctx, cancel := context.WithTimeout(context.Background(), QueryTimeOut)
	defer cancel()
...
func (u *Intern) Count(filter bson.M) (int64, error) {
	ctx, cancel := context.WithTimeout(context.Background(), QueryTimeOut)
	defer cancel()
