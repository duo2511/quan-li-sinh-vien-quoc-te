...
func AllSubjectScores(c *gin.Context) {
	data := bson.M{}
	var err error
	entry := collections.SubjectScore{}
	entries := collections.SubjectScores{}
	...
	fmt.Print("abcd1111111111", len(entries))

	controllers.ResponseSuccess(c, http.StatusOK, "Lấy dữ liệu thành công", data)
}

func ListSubjectScores(c *gin.Context) {
	data := bson.M{}
	var err error
	entry := collections.SubjectScore{}
	entries := collections.SubjectScores{}
	...
	filter := bson.M{"student": idStudent}
...
func CreateSubjectScore(c *gin.Context) {
	data := bson.M{}
	var err error
	entry := collections.SubjectScore{}
	idStudent, _ := primitive.ObjectIDFromHex(c.Param("idstudent"))
...
func GetSubjectScore(c *gin.Context) {
	data := bson.M{}
	var err error
	entry := collections.SubjectScore{}
	...
	data["entry"] = entry
	controllers.ResponseSuccess(c, http.StatusOK, "Lấy dữ liệu thành công 2", data)
}

func UpdateSubjectScore(c *gin.Context) {
	data := bson.M{}
	var err error
	var entry, entryUpdate collections.SubjectScore
...
func DeleteSubjectScore(c *gin.Context) {
	data := bson.M{}
	var err error
	entry := collections.SubjectScore{}
