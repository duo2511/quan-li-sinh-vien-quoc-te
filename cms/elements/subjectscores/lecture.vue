...
<template>
  <a-form-model-item prop="lecture" name="lecture" label="Cố vấn học tập">
    <a-select mode="default" v-model:value="entry.lecture.name" show-search placeholder="Chọn giảng viên..."
      style="width: 100%" :not-found-content="loading ? undefined : null" @search="fetchData" @change="handleChange">
...
export default {
  ...
  methods: {
    async getData(search = "") {
      ...
      console.log('ok3', this.entry.lecture);
      this.loading = false;
    },
    handleChange(entry) {
      let self = this;
