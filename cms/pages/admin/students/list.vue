<template>
  <div>
   ...
   <h4>Tổng số sinh viên : {{ countStudents() }}</h4>
      <p>CNTT : {{ countStudentsByDepartment("CNTT") }}</p>
      <p>ATTT : {{ countStudentsByDepartment("ATTT") }}</p>
      <p>CN Đa Phương Tiện : {{ countStudentsByDepartment("CN Đa Phương Tiện") }}</p>
...
export default {
  ...
  methods: {
    ...
    }, countStudentsByDepartment(name) {
      return this.raw_entries.filter(entry => entry.department.name === name).length;
    },
