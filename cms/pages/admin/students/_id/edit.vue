                ...
                <a-col :md="12">
                      <a-row :gutter="16">
                        <a-col :md="12">
                         ...
                         <a-col :md="12">
                          <a-form-model-item prop="classname" name="classname" label="Lớp học ">
                              <a-input  placeholder="..." />
                          </a-form-model-item>
                        </a-col> 
                      </a-row>
  ...
  methods: {
    ...
    encodeTag() {
      ...
      if (file) {
        ...
        reader.onload = (e) => {
          this.entry.avatar = e.target.result;
          console.log("aa121", e.target.result)
        }
      }
    },
    async getData() {
      this.loading = true;
      console.log("hello1 ", this.$route.params.id)
      let response = await this.$store.dispatch("articles/GetStudent", this.$route.params.id);
    ...
    async SubmitForm(status = "draft") {
      this.$refs.FormData.validate(async (valid) => {
        if (valid) {
          ...
          let response = await this.$store.dispatch("articles/UpdateStudent", this.entry);
