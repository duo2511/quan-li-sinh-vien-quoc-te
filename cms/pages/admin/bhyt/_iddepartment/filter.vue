...
export default {
    ...
    methods: {
        ...
        async getData() {
           ...
           // console.log('a22a', this.raw_entries)
            let response2 = await this.$store.dispatch("articles/ListStudents", this.pagination);
            this.student_entries = response2.data.entries;
            this.student_entries = this.student_entries.filter(entry => entry.department.id === this.$route.params.iddepartment)
           // console.log('a11a', this.student_entries)
